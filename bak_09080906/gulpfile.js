    ...
    html: function() {
        ...
        var combined = combiner.obj([
            ...
            cheerio(function($, file) { //进入.html文件
                ...
                //修改background-image路径，在html中以style方式写入的背景图片
...
var watch = {
	//自动监听进行JS检查以及自动生成文档
	/* markdown的注释标签 http://need-faith.iteye.com/blog/2019702
	@author
	@classdesc
	@description
	@param
		 ...
		 gulp.watch(src.dir + '/**/*.js',function(event) {
		 	var _src = event.path;
		 		if(src.jsdoc){
	            	...
	            	jsdocConfig.opts.destination = "docs/"+getFile.modDir(_src)+"/"+getFile.name(_src);
	            	del(['./docs/' +getFile.modDir(_src)+"/"+getFile.name(_src)]);
            	}
            gulp.src(_src)
