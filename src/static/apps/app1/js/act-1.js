require(['$', 'msui', 'infiniteScroll'], function($, Msui, infiniteScroll) {
    ...
    var scroll = new infiniteScroll({
        ...
        ajaxUrl:_CONF.sDomain()+'/apps/v1.0/js/act-1.json', // ajax请求地址
        getItems:30, // 每次ajax获取的条目
        maxItems:30, //当前页累计可加载的条目
        ...
        callback:function(data){
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                var item = data[i];
                html += '<div class="col-50 product-item"><a href="#"><img src="' + item.pro_img + '" class="pro-img"><div class="pro-info"><span class="pro-name">' + item.pro_id + item.pro_name + '</span><span class="pro-price"><i class="pro-oldPrice">¥'+item.pro_oldPrice+'</i><em>¥</em>' + item.pro_price + '</span><i class="iconfont icon-gouwuche"></i></div></a></div>'
            }
            $('.product-list-content .row').append(html); 
        }
    })
    /* 注册'infinite'事件处理函数 */
